/// socket  - 관리자로부터 받는 데이터
/// socket2 - 관리자로 보내는 데이터
/// socket3 - 디스플레이에 보내는 데이터
...
//파일 쓰기
void file(int data, string location){
    ofstream output("/home/hyun/" + location);
    output << data;
    output.close();
}
...
std_msgs::Int16 left, right, turn, go, back, ultra, init, rotate;
};
